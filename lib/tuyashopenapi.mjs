...
class TuyaSHOpenAPI {
  ...
  async getDevicesFunctions(devIds = []) {
    ...
    if (!res?.success) {
      this.log?.warn?.(`getDevicesFunctions failed (non-fatal): ${JSON.stringify(res)}`);
      return [];
    }
    return res.result ?? [];
  }
  ...
  async request(method, path, params = null, body = null) {
    ...
    for (let attempt = 0; attempt < timeouts.length; attempt++) {
      ...
      try {
        const res = await fetch(`${this.endpoint}${requestUrl}`, {
        ...
        this.log?.log?.(
          `TuyaSHOpenAPI response: ${JSON.stringify(data)} path=${path}`,
        );
        return data;
      } catch (err) {
        if (
          ...
          this.log?.log?.(`TuyaSHOpenAPI request timed out, retrying...`);
          continue;
        }
        throw err;
      }
