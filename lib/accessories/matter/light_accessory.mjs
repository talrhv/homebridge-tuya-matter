...
// נוסף פרמטר קטגוריה כדי לדעת לתרגם את הנתונים לפי הסכמה של wg2
function readTuyaHsColor(source, colorCode, deviceCategory) {
  ...
  // הגדרת המקסימום: אם זה wg2, המקסימום הוא תמיד 1000 לפי המפרט
  const svMax = (deviceCategory === "wg2" || isV2Code(colorCode) || rawSat > 255 || rawVal > 255) ? 1000 : 255;
...
// בניית הפקודה ל-Tuya לפי סוג המכשיר
function encodeTuyaHsColor(colorCode, hueDegrees, saturationPercent, valuePercent, deviceCategory) {
  const h = clamp(Math.round(hueDegrees), 0, 360);
  
  // תמיכה ישירה במפרט של wg2
  if (deviceCategory === "wg2") {
    const s = clamp(Math.round((saturationPercent / 100) * 1000), 0, 1000);
    const v = clamp(Math.round((valuePercent / 100) * 1000), 0, 1000);
    return JSON.stringify({ h, s, v });
  }

  // מכשירים רגילים (מנורות דור 1 ו-2)
  const isV2 = isV2Code(colorCode);
  const svMax = isV2 ? 1000 : 255;
    ...
    const setLevelPercent = async (percent, options = {}) => {
      ...
      if (context.colorCode && (!context.brightnessCode || workMode === "colour")) {
        ...
        commands.push({
          ...
          value: encodeTuyaHsColor(context.colorCode, nextColorNormalized.h, nextColorNormalized.s, nextColorNormalized.v, deviceCategory),
        });
