...
function readTuyaHsColor(source, context) {
  if (!context.colorCode) {
    return null;
  }

  const raw = getStatusValue(source, context.colorCode);
  ...
  // Detect 1000 scale: V2 codes, wg2 category, or if we read a value > 255
  const svMax = (isV2Code(context.colorCode) || context.category === "wg2" || rawSat > 255 || rawVal > 255) ? 1000 : 255;
...
function encodeTuyaHsColor(context, hueDegrees, saturationPercent, valuePercent) {
  const isV2 = isV2Code(context.colorCode);
  
  // wg2 devices expect the 1000 scale even if they don't have the _v2 suffix
  const svMax = (isV2 || context.category === "wg2") ? 1000 : 255;
    ...
    const setLevelPercent = async (percent, options = {}) => {
      ...
      if (context.colorCode && (!context.brightnessCode || workMode === "colour" || workMode === "color")) {
        ...
        if (context.workModeCode) {
          commands.push({ code: context.workModeCode, value: getTargetColorMode(context) });
        }
        
        commands.push({
        code: context.colorCode,
        value: encodeTuyaHsColor(context, nextColorNormalized.h, nextColorNormalized.s, nextColorNormalized.v),
      });
