...
// --- FIX 1: הפיכת ה-JSON ל-Hex String תקני עבור מנורות דור 1 ---
function encodeTuyaHsColor(colorCode, hueDegrees, saturationPercent, valuePercent) {
  const isV2 = isV2Code(colorCode);
  // מקפידים להשתמש בסקאלה של 1000 תמיד כדי למנוע עמעום חריג של הלדים
  const svMax = 1000; 
  ...
  // מכשירים מדור 1 (colour_data) חייבים לקבל את זה כמחרוזת של 12 תווים Hex
...
// --- FIX 2: פרוטוקול Base64 סודי עבור פסי לד מסוג WG2 (הנדסה לאחור מהלוגים) ---
function getWg2Base64(h, s, v) {
  ...
  // מבנה הבתים הנדרש לפס לד: 00 01 00 14 00 + Hue + Sat + Val
...
export default class LightMatterAccessory {
  ...
  static create(platform, bridge, device) {
    ...
    const context = {
      ...
      deviceId: device.id,
      category: device.category, // שמירת הקטגוריה לזיהוי פס הלדים
      powerCode,
      brightnessCode,
      tempCode,
      colorCode,
  ...
  static buildHandlers(platform, bridge, context, discoveredDevice) {
    ...
    const setLevelPercent = async (percent, options = {}) => {
      ...
      if (context.colorCode && (!context.brightnessCode || workMode === "colour")) {
        ...
        // הזרקת הפרוטוקול המיוחד (DP 61) לפסי לד
        if (context.category === "wg2") {
