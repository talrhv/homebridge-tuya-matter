...
function parseFunctionValues(source, code) {
  const entry = listFunctions(source).find((candidate) => candidate?.code === code);
  if (!entry?.values || typeof entry.values !== "string") {
    return null;
  }
  try {
    return JSON.parse(entry.values);
  } catch {
    return null;
  }
}

function getEnumRangeForCode(source, code) {
  const parsed = parseFunctionValues(source, code);
  return Array.isArray(parsed?.range) ? parsed.range : null;
}
...
function detectColorPayloadStyle(source, colorCode, deviceCategory) {
  const raw = colorCode ? getStatusValue(source, colorCode) : undefined;

  if (deviceCategory === "wg2") {
    return { mode: "object", svMax: 1000 };
  }

  if (raw && typeof raw === "object") {
    const s = Number(raw.s ?? 0);
    const v = Number(raw.v ?? 0);
    return { mode: "object", svMax: s > 255 || v > 255 ? 1000 : 255 };
  }
  ...
  if (typeof raw === "string") {
    ...
    if (/^[0-9a-fA-F]{12}$/.test(raw.trim())) {
      if (isV2Code(colorCode)) {
        return { mode: "object", svMax: 1000 };
      }

      const parsed = parseHexColorData(raw);
      if (parsed && (parsed.s > 255 || parsed.v > 255)) {
        return { mode: "hex", svMax: 1000 };
      }

      return { mode: "hex", svMax: 255 };
    }
  }
...
function readTuyaHsColor(source, colorCode, deviceCategory) {
  if (!colorCode) {
    return null;
  }
  ...
  const style = detectColorPayloadStyle(source, colorCode, deviceCategory);
  const svMax = style.svMax;
...
function encodeTuyaHsColor(source, colorCode, hueDegrees, saturationPercent, valuePercent, deviceCategory) {
...
function resolveWorkMode(source, context) {
  ...
  if (context.supportsColor && !context.supportsColorTemp) {
    return resolveModeToken(source, context, "color");
  }

  if (context.supportsColorTemp) {
    return resolveModeToken(source, context, "white");
  }
...
function isColorMode(value) {
  ...
  return normalized === "colour" || normalized === "color";
}

function resolveModeToken(source, context, desired) {
  const current = context.workModeCode ? getStatusValue(source, context.workModeCode) : undefined;
  if (typeof current === "string" && current) {
    const normalized = current.toLowerCase();
    if (desired === "color" && (normalized === "colour" || normalized === "color")) {
      return current;
    }
    if (desired === "white" && (normalized === "white" || normalized === "light_white")) {
      return current;
    }
  }

  const range = context.workModeCode ? getEnumRangeForCode(source, context.workModeCode) : null;
  if (Array.isArray(range) && range.length > 0) {
    if (desired === "color") {
      const found = range.find((entry) => entry === "colour" || entry === "color");
      if (found) {
        return found;
      }
    }
    if (desired === "white") {
      const found = range.find((entry) => entry === "white" || entry === "light_white");
      if (found) {
        return found;
      }
    }
  ...
  return desired === "color" ? "colour" : "white";
}

function getCurrentBrightnessPercent(source, context, deviceCategory) {
  const workMode = resolveWorkMode(source, context);

  if (context.colorCode && (!context.brightnessCode || isColorMode(workMode))) {
    const hsColor = readTuyaHsColor(source, context.colorCode, deviceCategory);
    if (hsColor) {
      return hsColor.v;
    }
  }
...
function buildInitialColorControlState(platform, source, context, deviceCategory) {
  const state = {};
  ...
  const workMode = resolveWorkMode(source, context);
  state.colorMode = workMode === resolveModeToken(source, context, "white") && context.supportsColorTemp
...
export default class LightMatterAccessory {
  ...
  static create(platform, bridge, device) {
    ...
    const deviceCategory = String(context.category).toLowerCase();
    ...
    if (supportsLevel) {
      accessory.clusters.levelControl = {
        currentLevel: percentToMatterLevel(getCurrentBrightnessPercent(device, context, deviceCategory)),
        minLevel: 1,
        maxLevel: 254,
      };
    }
    ...
    if (supportsColor || supportsColorTemp) {
      accessory.clusters.colorControl = buildInitialColorControlState(platform, device, context, deviceCategory);
    }
  ...
  static buildHandlers(platform, bridge, context, discoveredDevice) {
    ...
    const getDeviceCategory = () => String(context.category || getSource()?.category || "").toLowerCase();
    ...
    const setLevelPercent = async (percent, options = {}) => {
      ...
      if (context.colorCode && (!context.brightnessCode || isColorMode(workMode))) {
        const currentColor = optimisticCache.color ?? readTuyaHsColor(source, context.colorCode, deviceCategory) ?? { h: 0, s: 0, v: 100 };
        const nextColorNormalized = { h: currentColor.h, s: currentColor.s, v: safePercent };

        if (context.workModeCode) {
          commands.push({ code: context.workModeCode, value: resolveModeToken(source, context, "color") });
        }

        commands.push({
          code: context.colorCode,
          value: encodeTuyaHsColor(source, context.colorCode, nextColorNormalized.h, nextColorNormalized.s, nextColorNormalized.v, deviceCategory),
        });
    ...
    const setHueSaturation = async (hueDegrees, saturationPercent, valuePercent = null) => {
      ...
      const nextValue = valuePercent == null
        ? (optimisticCache.brightness ?? getCurrentBrightnessPercent(source, context, deviceCategory))
    ...
    if (context.supportsColorTemp && context.tempCode) {
      handlers.colorControl.moveToColorTemperatureLogic = async ({ colorTemperatureMireds }) => {
        ...
        const commands = [];
        if (context.workModeCode) {
          commands.push({ code: context.workModeCode, value: resolveModeToken(source, context, "white") });
        }
        commands.push({
    ...
    if (context.supportsColor && context.colorCode) {
      ...
      handlers.colorControl.moveToColorLogic = async ({ targetX, targetY }) => {
        const source = getSource();
        const deviceCategory = getDeviceCategory();
        const brightnessPercent = optimisticCache.brightness ?? getCurrentBrightnessPercent(source, context, deviceCategory);
  ...
  static async sync(platform, bridge, accessory, device) {
    ...
    const context = accessory.context ?? {};
    const deviceCategory = String(context.category || device?.category || "").toLowerCase();
    ...
    if ((context.supportsColorTemp || context.supportsColor) && platform.api.matter.clusterNames.ColorControl) {
      ...
      if (context.supportsColor && context.colorCode) {
        const hsColor = readTuyaHsColor(device, context.colorCode, deviceCategory);
        if (hsColor) {
      ...
      colorState.colorMode = workMode === resolveModeToken(device, context, "white") && context.supportsColorTemp
        ? getColorModeValue(platform, "temperature")
        : getColorModeValue(platform, "hs");
