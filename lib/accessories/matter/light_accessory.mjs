...
function encodeTuyaHsColor(colorCode, hueDegrees, saturationPercent, valuePercent) {
  const svMax = isV2Code(colorCode) ? 1000 : 255;
  return {
    h: clamp(Math.round(hueDegrees), 0, 360),
    s: clamp(Math.round((clamp(saturationPercent, 0, 100) / 100) * svMax), 0, svMax),
    v: clamp(Math.round((clamp(valuePercent, 0, 100) / 100) * svMax), 0, svMax),
  };
}
...
function buildInitialColorControlState(platform, source, context) {
  ...
  // --- FEATURE MAP FIX ---
  // אנחנו מכריחים את Matter.js להגדיר בדיוק אילו תכונות הקלאסטר הזה מריץ.
  // זה מה שימנע ממנו לחשוף את ה-Attributes של הצבע ל-Apple HomeKit.
  state.features = {
  ...
  // הזרקת ערך ביטמאפ ישיר כגיבוי (0x10 זה רק טמפרטורה, 0x0F זה צבע מלא)
  state.featureMap = context.supportsColor ? 0x0F : 0x10;
  // -----------------------

  if (context.supportsColorTemp && context.tempCode) {
    const { minMireds, maxMireds } = getColorTempBounds();
    const colorTemperatureMireds = getCurrentColorTempMireds(source, context.tempCode);
    ...
    state.colorTemperatureMireds = colorTemperatureMireds;
    
    // מעדכן את היכולות הפנימיות לטמפרטורה בלבד (ביט 4)
    state.colorCapabilities = context.supportsColor ? 25 : 16;
  }
  ...
  const workMode = resolveWorkMode(source, context);
  state.colorMode = workMode === "white"
...
export default class LightMatterAccessory {
  ...
  const getSource = () => bridge.latestDevices.get(context.deviceId) ?? discoveredDevice;
    ...
    // --- OPTIMISTIC CACHE FIX ---
    // שומר את הנתונים המנורמלים באחוזים (0-100) כדי שיתאימו ל-readTuyaHsColor
    const optimisticCache = {
      ...
      update(normalizedColor, brightnessPercent) {
        ...
        // מחיקת הקאש לאחר 3 שניות - הזמן הממוצע עד שסנכרון ה-MQTT מגיע
    ...
    const setLevelPercent = async (percent, options = {}) => {
      ...
      if (context.colorCode && (!context.brightnessCode || workMode === "colour")) {
        // שימוש בקאש האופטימי לפני המשיכה מה-API
        ...
        // יצירת האובייקט המנורמל עבור הקאש
        const nextColorNormalized = { h: currentColor.h, s: currentColor.s, v: safePercent };
        
        commands.push({
          code: context.colorCode,
          value: encodeTuyaHsColor(context.colorCode, nextColorNormalized.h, nextColorNormalized.s, nextColorNormalized.v),
        });
  ...
  static async sync(platform, bridge, accessory, device) {
    ...
    if ((context.supportsColorTemp || context.supportsColor) && platform.api.matter.clusterNames.ColorControl) {
      ...
      colorState.colorMode = workMode === "white"
        ? getColorModeValue(platform, "temperature")
        : getColorModeValue(platform, "hs");
