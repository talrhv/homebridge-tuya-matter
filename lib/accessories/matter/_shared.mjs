...
/**
 * Re-attach command handlers to a composed accessory's child endpoints IN PLACE,
 * matched by their stable part id, so sub-endpoint numbers stay stable across
 * Homebridge restarts. Re-creating the `parts` array on every boot re-numbers the
 * child endpoints, which breaks Matter pairing (room reset + UnsupportedEndpoint).
 * Each caller passes a `buildPart(code, index)` factory that returns a full part
 * object; on cached accessories we only swap in its fresh `handlers` closures and
 * leave the restored `id`/`displayName`/`clusters` untouched. The array is only
 * built from scratch as a fallback when no parts were restored from cache.
 */
export function rebindPartsInPlace(accessory, codes, typeId, buildPart) {
  if (!Array.isArray(accessory.parts) || accessory.parts.length === 0) {
    accessory.parts = codes.map((code, index) => buildPart(code, index));
    return;
  }

  codes.forEach((code, index) => {
    const id = toPartId(code, typeId);
    const part = accessory.parts.find((candidate) => candidate?.id === id);
    if (part) {
      part.handlers = buildPart(code, index).handlers;
    } else {
      // A code with no cached part (shape drift) — append so it still works.
      accessory.parts.push(buildPart(code, index));
    }
  });
}
