...
export default class SwitchMatterAccessory {
  ...
  static rebind(platform, bridge, accessory) {
    ...
    // Handlers only — persisted Matter state wins on restart; live values arrive
    // via sync(). Rewriting clusters here would clobber the restored on/off state.
    accessory.handlers = {
      ...(accessory.handlers || {}),
      onOff: {
        on: async () =>
    ...
    rebindPartsInPlace(accessory, gangCodes.slice(1), this.id, (code, index) => ({
      id: toPartId(code, this.id),
      displayName: partLabel("Switch", index + 1),
      deviceType: accessory.deviceType,
