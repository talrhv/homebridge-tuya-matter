...
class WindowCoveringAccessory extends BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig, options = {}) {
    const { Categories, Service } = platform.api.hap;

    // ברירת מחדל: וילון. עבור פותחן חלון (mc) משתמשים בשירות Window.
    const useWindow = options.window === true;

    super(
      platform,
      homebridgeAccessory,
      deviceConfig,
      useWindow ? Categories.WINDOW : Categories.WINDOW_COVERING,
      useWindow ? Service.Window : Service.WindowCovering,
    );
