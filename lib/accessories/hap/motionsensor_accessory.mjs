...
class MotionSensorAccessory extends BaseAccessory {
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      ...
      switch (statusMap.code) {
        case "pir":
          this._handleMotionUpdate(rawValue === "pir", isRefresh);
          break;

        case "pir_state":
          this._handleMotionUpdate(
            rawValue === true || rawValue === 1 || rawValue === "pir" || rawValue === "motion",
            isRefresh,
          );
          break;
        ...
        case "presence_state":
          this._handleMotionUpdate(
            rawValue === true || rawValue === 1 || rawValue === "presence",
            isRefresh,
          );
          break;
  ...
  _handleMotionUpdate(isMotion, isRefresh) {
    ...
    } else if (!isMotion && this._freezeTimer) {
      // Override timer is running; ignore "no motion" from sensor until it expires.
    } else {
