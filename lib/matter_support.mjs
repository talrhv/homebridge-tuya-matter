 ...
 async registerDevices(devices = []) {
    ...
    for (const device of devices) {
      ...
      if (existing && this.restoringAccessories.has(existing.UUID)) {
        ...
        // Do NOT add to pendingRegistrations here — we are not re-registering
        // cached accessories, so there is no pending Matter stack operation to gate on.
        // The postRegistrationTimers entry (added below) is the correct guard.
      }
    }

    // ── Re-attach cached accessories WITHOUT re-registering them ────────────
    // Calling registerPlatformAccessories() on an already-commissioned accessory
    // tells the Home app to treat it as a brand-new device, which resets its
    // room placement to the bridge on every Homebridge restart.
    //
    // Correct pattern (Homebridge Matter docs):
    //   • Cached accessories  → mutate in-place, then updatePlatformAccessories()
    //   • Brand-new devices   → registerPlatformAccessories()  (handled in Pass 2)
    if (restoringBatch.length > 0) {
      const toUpdateList = [];

      for (const { existing, created, device, matterType } of restoringBatch) {
        // Merge any fresh context (new capabilities, category, etc.) into the
        // cached accessory object WITHOUT replacing its commissioned identity.
        ...
        // Rebind live handlers and reapply name support on the cached object.
        this.rebindHandlers(existing, device, matterType);

        // Gate state-push messages until the Matter stack has settled.
        this.schedulePostRegistrationSync(existing, { restoring: true, delayMs: 250 });

        toUpdateList.push(existing);
      }

      // Persist context / metadata changes without re-commissioning the accessory.
      // updatePlatformAccessories() does not announce a new pairing to the Home app,
      // so room assignments and user renames are preserved across restarts.
      if (typeof this.api?.matter?.updatePlatformAccessories === "function") {
        try {
          await this.api.matter.updatePlatformAccessories(toUpdateList);
        } catch (error) {
          this.log.warn(
            `[Matter] Failed to persist restored accessory metadata: ${errorText(error)}`,
          );
        }
      }
    }
