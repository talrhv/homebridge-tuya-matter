...
export default class TuyaMatterBridge {
  ...
  getAccessoryUuid(accessory) {
    return accessory?.UUID || accessory?.uuid || null;
  }

  restoreAccessory(accessory) {
    const uuid = this.getAccessoryUuid(accessory);
    if (!uuid) return;

    // Some Homebridge betas expose restored Matter accessories with `uuid`
    // instead of `UUID`. Normalise to `UUID` so the rest of the bridge logic
    // can treat restored and newly-created accessories the same way.
    if (!accessory.UUID) {
      accessory.UUID = uuid;
    }
    ...
    this.restoringAccessories.add(uuid);
  }
  ...
  async handleNodeLabelWrite(accessory, target, request, options = {}) {
    ...
    await this.persistStoredAccessory(accessory);
  ...
  async persistStoredAccessory(accessory) {
    const uuid = this.getAccessoryUuid(accessory);
    if (!uuid || typeof this.api?.matter?.updatePlatformAccessories !== "function") {
      return;
    }

    try {
      await this.api.matter.updatePlatformAccessories([
        {
          UUID: uuid,
          displayName: accessory?.displayName,
          context: cloneJson(accessory?.context, {}) || {},
        },
      ]);
    } catch (error) {
      this.log.debug?.(
        `[Matter][${uuid}] Failed to persist cached accessory metadata: ${errorText(error)}`,
      );
    }
  }
 ...
 async registerDevices(devices = []) {
    ...
    for (const device of devices) {
      try {
        ...
        const wasRestoring = this.restoringAccessories.has(existing.UUID);

        // Homebridge Matter docs pattern:
        // - configureMatterAccessory()/restoreAccessory() gives us the cached accessory
        // - cached accessories are automatically re-registered by Homebridge
        // - registerPlatformAccessories() should only be used for NEW accessories
        //
        // Replacing the restored accessory with a fresh object from create()
        // causes HomeKit / Apple Home to treat the bridged endpoint as newly added,
        // which is why accessories jump back into the bridge room after a restart.
        // Keep the restored accessory object, rebind handlers, refresh metadata,
        // and only register brand-new accessories below.
  ...
  async refreshStoredMetadata(existing, desired, device, matterType) {
    ...
    if (changed) {
      ...
      await this.persistStoredAccessory(existing);
    }
