...
async registerDevices(devices = []) {
  ...
  // Keep restored accessories on their cached object identity/shape.
  // Re-registering a freshly recreated object can make Apple Home treat
  // them like newly published children of the bridge.
  ...
  for (const device of devices) {
    ...
    if (existing && this.restoringAccessories.has(existing.UUID)) {
      ...
      if (shape.changed) {
        ...
        const changed = await this.refreshStoredMetadata(
          existing,
          created,
          device,
          matterType,
        ...
        if (
          changed &&
          typeof this.api?.matter?.updatePlatformAccessories === "function"
        ) {
          try {
            await this.api.matter.updatePlatformAccessories([existing]);
          } catch (error) {
            this.log.warn(
              `[Matter][${device.id}] Failed to persist cached accessory metadata: ${errorText(error)}`,
            );
          }
        }
      }
  ...
  if (restoringBatch.length > 0) {
    try {
      ...
      for (const accessory of restoringBatch) {
        this.pendingRegistrations.delete(accessory.UUID);
      }
    }

    for (const accessory of restoringBatch) {
      if (this.pendingRegistrations.has(accessory.UUID)) {
