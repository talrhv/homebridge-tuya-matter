...
async registerPlatformAccessory(accessory) {
  ...
  // Mark the endpoint as live the instant registration succeeds. This is the
  // authoritative "the Matter endpoint exists" signal — every registration path
  // funnels through here, so runtime state syncs can rely on it.
  if (accessory?.UUID) {
...
async syncDeviceSnapshot(device) {
    ...
    // The Matter endpoint must actually be registered before we push state to
    // it. MQTT can deliver messages during the startup window (MQTT starts
    // before registerDevices completes), and pushing to an unregistered
    // endpoint throws "not registered or missing endpoint". The current state
    // is applied at registration time from the latest snapshot, so skipping
    // here loses nothing.
...
async safeUpdateAccessoryState(uuid, clusterName, patch, options = {}) {
  ...
  // Never push to an endpoint that isn't registered with the Matter framework
  // (e.g. a deferred override timer firing after the device was removed, or a
  // message racing the startup registration). Avoids "not registered or missing
  // endpoint" errors.
  if (!this.sessionRegistered.has(uuid)) {
    return false;
  }
