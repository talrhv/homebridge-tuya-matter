...
export default class TuyaMatterBridge {
 ...
 async registerDevices(devices = []) {
    ...
    // Register all restoring accessories in ONE call
    if (restoringBatch.length > 0) {
      const toRegisterList = restoringBatch.map(({ existing, created }) => ({
        ...created,
        UUID: existing.UUID,
      ...
      try {
        await this.api.matter.registerPlatformAccessories(
          this.pluginName, this.platformName, toRegisterList,
        );
      } catch (error) {
  ...
  async refreshStoredMetadata(existing, desired, device, matterType) {
    ...
    if (desired.displayName && existing.displayName !== desired.displayName) {
      ...
      const bridgedCluster = this.api.matter.clusterNames.BridgedDeviceBasicInformation || "bridgedDeviceBasicInformation";
      
      this.log.debug?.(`[Matter][${device.id}] Startup name sync: ${oldName} -> ${desired.displayName}`);

      await this.safeUpdateAccessoryState(
        existing.UUID,
        bridgedCluster,
        { nodeLabel: desired.displayName },
        { force: true }
      );
    }
...
rebindHandlers(accessory, discoveredDevice, matterType = null) {
    ...
    if (resolvedType?.rebind) {
      try {
        ...
        this.log.warn(`[Matter] Failed to rebind handlers for ${accessory?.displayName || accessory?.UUID}: ${errorText(error)}`);
      }
    }
    const bridgedCluster = this.api.matter.clusterNames.BridgedDeviceBasicInformation || "bridgedDeviceBasicInformation";
    if (!accessory.handlers) accessory.handlers = {};
    if (!accessory.handlers[bridgedCluster]) accessory.handlers[bridgedCluster] = {};

   accessory.handlers[bridgedCluster].nodeLabel = async (newName) => {
      this.log.info(`[Matter][${accessory.context?.deviceId}] Renamed in Home app to: ${newName}`);

      // Update local cache
      accessory.displayName = newName;
      if (accessory.context) accessory.context.displayName = newName;
...
async syncDeviceSnapshot(device) {
    ...
    // Use BridgedDeviceBasicInformation for bridged accessories
    const bridgedCluster = this.api.matter.clusterNames.BridgedDeviceBasicInformation || "bridgedDeviceBasicInformation";
    const liveName = device.name || accessory.displayName;
