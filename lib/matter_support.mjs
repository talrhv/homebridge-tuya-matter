  ...
  async refreshStoredMetadata(existing, desired, device, matterType) {
    ...
    if (!deepEqual(existing.context ?? {}, desiredContext ?? {})) {
      ...
      await this.safeUpdateAccessoryState(
        existing.UUID, 
        this.api.matter.clusterNames.BasicInformation, 
...
rebindHandlers(accessory, discoveredDevice, matterType = null) {
    ...
    // GLOBAL FIX: Inject the Name Write Handler for all accessories
    if (!accessory.handlers) accessory.handlers = {};
    if (!accessory.handlers.basicInformation) accessory.handlers.basicInformation = {};

    accessory.handlers.basicInformation.nodeLabel = async (newName) => {
