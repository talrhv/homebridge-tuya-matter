...
export default class TuyaMatterBridge {
  ...
  async handleNodeLabelWrite(accessory, target, request, options = {}) {
    ...
    // Persist the rename to the Homebridge cache file immediately.
    //
    // Without this, homeDisplayName (and partDisplayNames) only exist in memory.
    // Homebridge saves the Matter accessory cache once at startup — renames that
    // happen after that save are lost when Homebridge restarts.
    //
    // On the next boot, existing.context is loaded from the stale cache with no
    // homeDisplayName, so applyNameSupport falls back to the Tuya device name and
    // passes nodeLabel = <Tuya name> in the initial cluster states. The Matter
    // server pushes that changed nodeLabel to Apple Home via its active subscription,
    // which causes the Home app to:
    //   1. Overwrite the user's custom accessory name with the Tuya device name.
    //   2. Treat the accessory as a changed/new device and reset its room placement
    //      back to the bridge's default room.
    //
    // Calling updatePlatformAccessories here writes the updated context (with the
    // correct homeDisplayName / partDisplayNames) to the cache file immediately, so
    // the rename survives the next restart and nodeLabel is stable across reboots.
 ...
 async registerDevices(devices = []) {
    ...
    // to register accessories in the current session — even for already-commissioned
    // devices. Without this the server rejects updateAccessoryState calls with
    // "not registered in current session".
    //
    // Root cause of BOTH previous bugs:
    //
    //   Bug A — passing `existing` directly to registerPlatformAccessories:
    //     The cached accessory is JSON-deserialized on restore. Its `deviceType`
    //     (and each part's `deviceType`) are plain objects, not live Matter SDK
    //     class instances. The server calls `deviceType.with(...)` internally,
    //     which throws "TypeError: deviceType.with is not a function" and fails
    //     ALL accessory registrations.
    //
    //   Bug B — original code: `{ ...created, context: existing.context }`:
    //     The raw `existing.context` was used without merging in the fresh technical
    //     fields that `create()` produces (matterAccessoryType, category, etc.),
    //     causing stale context to accumulate across restarts.
    //
    // Correct hybrid approach:
    //   • deviceType / parts / clusters / handlers  →  from `created`
    //     (live SDK class instances; required by AccessoryManager.registerAccessory)
    //   • UUID                                      →  from `existing`
    //     (preserves the commissioned identity; HomeKit keeps room placement)
    //   • context                                   →  MERGED via buildPersistedContext
    //     (existing wins for user-set fields: homeDisplayName, partDisplayNames;
    //      created wins for technical fields: matterAccessoryType, category, etc.)
    if (restoringBatch.length > 0) {
      const toRegisterList = [];

      for (const { existing, created, device, matterType } of restoringBatch) {
        // Merge context: previousContext=existing preserves user-set display names;
        // rawContext=created.context brings in fresh technical fields.
        const mergedContext = this.buildPersistedContext(
          created.context,
          device,
          matterType,
          existing.context,
        ...
        // Build registration object from `created` (live SDK class instances for
        // deviceType / parts / clusters / handlers) stamped with the existing UUID
        // so the Matter server and HomeKit recognise it as the same commissioned device.
        const forRegistration = this.applyNameSupport(
          {
            ...created,
            UUID: existing.UUID,
        ...
        // Keep the in-memory accessory map consistent with what we are registering.
        existing.context = mergedContext;
        this.accessories.set(existing.UUID, forRegistration);
        this.deviceIndex.set(device.id, existing.UUID);

        toRegisterList.push(forRegistration);
      }
      ...
      for (const { existing, device, matterType } of restoringBatch) {
        ...
        // Operate on the forRegistration object now stored in this.accessories.
        const registered = this.accessories.get(existing.UUID) ?? existing;
        this.rebindHandlers(registered, device, matterType);
        this.schedulePostRegistrationSync(registered, { restoring: true, delayMs: 250 });
      }
    }
