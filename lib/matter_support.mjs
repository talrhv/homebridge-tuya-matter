    ...
    for (const device of devices) {
      try {
        ...
        if (this.restoringAccessories.has(existing.UUID)) {
          ...
          const toRegister = {
            ...created, // live DeviceType instances with .with() intact
            UUID: existing.UUID, // preserve paired UUID so Home app keeps its pairing
          ...
          try {
            ...
            // schedulePostRegistrationSync will clear pendingRegistrations
            // via the restoring:true path (which clears restoringAccessories instead —
            // so explicitly remove from pendingRegistrations here too)
          ...
          // Do not fall through to syncDeviceSnapshot — the post-registration timer handles it.
