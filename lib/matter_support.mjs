...
export default class TuyaMatterBridge {
  ...
  restoreAccessory(accessory) {
    ...
    this.applyNameSupport(accessory);
  ...
  async handleNodeLabelWrite(accessory, target, request, options = {}) {
    ...
    if (partId && Array.isArray(accessory.parts)) {
      const matchedPart = accessory.parts.find((part) => part?.id === partId);
      if (matchedPart) {
        matchedPart.displayName = nextName;
      }
    }
    ...
    await this.persistAccessoryMetadata(accessory, `rename${partId ? ` (${partId})` : ""}`);

    return nextName;
  }

  async persistAccessoryMetadata(accessory, reason = "update") {
    if (!accessory?.UUID || typeof this.api?.matter?.updatePlatformAccessories !== "function") {
      return false;
    }
    ...
    try {
      ...
      this.log.debug?.(
        `[Matter][${accessory.context?.deviceId || accessory.UUID}] Persisted accessory metadata after ${reason}.`,
      );
      return true;
    } catch (error) {
      this.log.warn(
        `[Matter][${accessory.context?.deviceId || accessory.UUID}] Failed to persist accessory metadata after ${reason}: ${errorText(error)}`,
      );
      return false;
    }
  }
    ...
    for (const device of devices) {
      ...
      try {
        ...
        if (this.restoringAccessories.has(existing.UUID)) {
          ...
          this.log.debug?.(
            `[Matter][${device.id}] Reusing cached Matter accessory: ${existing.displayName || existing.UUID}`,
          );
        }
        ...
        if (shape.changed) {
          ...
          const metadataChanged = await this.refreshStoredMetadata(
            existing,
            created,
            device,
            matterType,
          ...
          if (metadataChanged) {
            await this.persistAccessoryMetadata(existing, "startup metadata refresh");
          }
        }
