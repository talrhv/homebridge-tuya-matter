...
restoreAccessory(accessory,device) {
  ...
  buildPersistedContext(rawContext, device, matterType, previousContext = undefined) {
    ...
    // Do NOT persist the full live device (its `status` array changes every boot,
    // which would make refreshStoredMetadata() fire updatePlatformAccessories() on
    // every restart). Only stable identity/capability fields are persisted; live
    // state is re-fetched from latestDevices and applied via sync(). Also prune any
    // device snapshot left in context by older plugin versions.
    ...
    for (const device of devices) {
      try {
        ...
        if (existing) {
          ...
          // Re-attach command handlers onto the LIVE restored accessory object.
          // Homebridge automatically re-registers cached Matter accessories after
          // configureMatterAccessory(); calling registerPlatformAccessories() again
          // here would re-create the endpoint with a new number, which breaks
          // pairing (accessories reset to the default room + UnsupportedEndpoint
          // errors when controlled). Handlers are read lazily at command-invoke
          // time, so mutating the existing object in place is sufficient.
