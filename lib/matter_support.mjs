...
export default class TuyaMatterBridge {
  constructor(platform) {
    ...
    this.registeredRestorations = new Set();
    this.restoreRegistrationPromise = null;
    this.didFinishLaunching = false;

    if (typeof this.api?.on === "function") {
      this.api.on("didFinishLaunching", () => {
        this.didFinishLaunching = true;
        void this.flushCachedRestoreRegistration();
      });
  ...
  async flushCachedRestoreRegistration() {
    if (!this.isEnabled()) {
      return;
    }

    if (this.restoreRegistrationPromise) {
      await this.restoreRegistrationPromise;
      return;
    }

    const accessories = [];
    for (const uuid of this.restoringAccessories) {
      if (
        this.registeredRestorations.has(uuid) ||
    ...
    this.restoreRegistrationPromise = (async () => {
      try {
        ...
        for (const accessory of accessories) {
          this.registeredRestorations.add(accessory.UUID);
        }
      } catch (error) {
        this.log.error(
          `[Matter] Cached restore failed: ${errorText(error)}`,
        );
      } finally {
        for (const accessory of accessories) {
  ...
  async finalizeRestoredAccessory(existing, desired, device, matterType) {
    if (!existing?.UUID) {
      return;
    }
    ...
    if (shape.changed) {
      if (!this.structureWarnings.has(existing.UUID)) {
        ...
        this.log.warn(
          `[Matter][${device?.id || existing.UUID}] Cached accessory shape differs from the current implementation (${shape.reasons.join(", ")}). Keeping the cached accessory to preserve pairing. Remove and re-pair manually if you want the new shape applied.`,
        );
      }
    } else if (desired && device) {
    ...
    this.restoringAccessories.delete(existing.UUID);
    this.registeredRestorations.delete(existing.UUID);
  ...
  async registerDevices(devices = []) {
    ...
    await this.flushCachedRestoreRegistration();
    ...
    for (const device of devices) {
      ...
      if (
        existing &&
        this.restoringAccessories.has(existing.UUID) &&
        !this.registeredRestorations.has(existing.UUID)
      ) {
    ...
    if (restoringBatch.length > 0) {
      try {
        await this.api.matter.registerPlatformAccessories(
          this.pluginName,
          this.platformName,
          restoringBatch.map(({ existing }) => existing),
        );

        for (const { existing } of restoringBatch) {
          this.registeredRestorations.add(existing.UUID);
        }
      } catch (error) {
        this.log.error(`[Matter] Batch restore failed: ${errorText(error)}`);
      } finally {
        for (const { existing } of restoringBatch) {
    ...
    for (const device of devices) {
      try {
        ...
        if (existing && this.restoringAccessories.has(existing.UUID)) {
          await this.finalizeRestoredAccessory(
            existing,
            created,
            device,
            matterType,
    ...
    const timer = setTimeout(
      async () => {
        ...
        if (restoring) {
          ...
          this.registeredRestorations.delete(uuid);
        } else {
          this.pendingRegistrations.delete(uuid);
        }
  ...
  cleanup() {
    ...
    this.registeredRestorations.clear();
    this.restoreRegistrationPromise = null;
