  ...
  try {
    ...
    for (const [uuid, cached] of cachedEntries) {
      try {
        ...
        // desired.context overrides it so fresh config-derived values (overrideTime,
        // hasInternalTimeSensor) take effect at registration on the first restart
        // after a config change — without any runtime fix-up that could clobber the
        // live endpoint.
...
async registerPlatformAccessory(accessory) {
  const result = await this.api.matter.registerPlatformAccessories(
      this.pluginName,
      this.platformName,
      [accessory],
    );
  // Mark the endpoint as live the moment registration succeeds. Every
  // registration path funnels through here, so runtime state syncs can rely on
  // this as the authoritative "the Matter endpoint exists" signal.
  if (accessory?.UUID) {
    this.sessionRegistered.add(accessory.UUID);
  }
  return result;
}
...
async refreshStoredMetadata(existing, desired, device, matterType) {
  ...
  // NOTE: Do NOT call this.api.matter.updatePlatformAccessories() here.
  // Homebridge's updatePlatformAccessories replaces its internal accessory record
  // (which holds the live `.endpoint`) with the plugin object we pass — and our
  // object has no `.endpoint`. That wipes the endpoint, after which every
  // updateAccessoryState throws "not registered or missing endpoint" and the
  // device dies until the next restart. There is no public API to update cached
  // metadata while preserving the live endpoint (getAccessory strips it).
  // The correct context is already persisted to the Matter cache at registration
  // time (finalizeAccessoryRegistration), so refreshing here is only an in-memory
  // update for the current session's runtime sync().
...
async syncDeviceSnapshot(device) {
    ...
    // The Matter endpoint must be registered before we push state to it. MQTT
    // can deliver messages during the startup window (MQTT starts before
    // registerDevices finishes), and pushing to an unregistered endpoint throws
    // "not registered or missing endpoint". Current state is applied at
    // registration time from the snapshot, so skipping here loses nothing.
    if (!this.sessionRegistered.has(uuid)) return;
