    ...
    try {
      ...
      if (this.datapointTopics && Array.isArray(payload?.status)) {
        for (const item of payload.status) {
          ...
          const dpTopic = `${this.topicPrefix}/${devId}/${item.code}`;
          this.client.publish(dpTopic, String(value), {
            qos: 0,
            retain: this.retain,
          });
          published.push(`${dpTopic}=${value}`);
        }
      }

      this.log?.debug?.(`[MQTT publish] published: ${published.join(", ")}`);
    } catch (error) {
      this.log?.error?.(
